...
deepseek_model = "deepseek/deepseek-r1-0528:free" # Default model, can be overridden by user
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                try:
                    ...
                    print(f"DeepSeekAgent ({self.player_name}): json.loads failed for initial response. Trying ast.literal_eval. Error: {e_json}")
                    try:
                        # Sanitize Python bools/None to JSON true/false/null
                        ...
                        print(f"DeepSeekAgent ({self.player_name}): Successfully parsed initial response with ast.literal_eval after sanitization.")
                    except (ValueError, SyntaxError) as e_ast:
                        print(f"DeepSeekAgent ({self.player_name}): ast.literal_eval also failed for initial response. Error: {e_ast}. Content: '{action_data_str}'")
